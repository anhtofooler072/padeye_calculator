...
**Role:** Act as an expert frontend developer and structural engineer specializing in offshore lifting operations. 
**Task:** Build a modern, interactive React web application that calculates and verifies Padeye Design Checks based on the engineering formulas provided below.
**Tech Stack:** React, TypeScript, Tailwind CSS, and `shadcn/ui`.

**App Requirements:**
1. **User Interface:** Create a clean dashboard using `shadcn/ui` components (Cards, Inputs, Labels, Badges, and Alerts).
2. **Inputs Section:** Create a form for the user to input the variables listed in the "Nomenclature" section (e.g., Sling Load, material yield strength, dimensions, sling angle). Use sensible defaults for testing.
3. **Calculations:** Implement the formulas exactly as specified in the "Formulas" section below. Ensure correct unit conversions if necessary.
4. **Results & Unity Checks:** Display the results of the dimension checks and strength unity checks. Use visual indicators (e.g., green `shadcn` Badges for passing checks where $\le 1.0$ or $> 0$, and red for failing checks). 
5. **Layout:** Split the layout logically (e.g., inputs on the left/top, real-time calculation results and safety warnings on the right/bottom).

Please generate the React components, the necessary state management (e.g., using `useState` or `useReducer`), and the calculation logic based on the document below.
...
* **Required Tensile Stress:** $$\sigma_u = \frac{F_1}{A_{I-I}}$$
* **Allowable Tensile Rupture:** $$\sigma_n = 0.5 \times F_u$$
* **Allowable Tensile Yielding:** $$\sigma_n = 0.6 \times F_y$$
...
* **Axial Stress:** $$\sigma_{u1} = \frac{F_1 \sin(\alpha)}{A_{II-II}}$$
* **In-plane Bending Stress:** $$\sigma_{u2} = \frac{F_1 \cos(\alpha)H}{S_{z,II-II}}$$
* **Out-of-plane Bending Stress:** $$\sigma_{u3} = \frac{F_2 H}{S_{x,II-II}}$$
* **Combined Tensile Stress:** $$\sigma_u = \sigma_{u1} + \sigma_{u2} + \sigma_{u3}$$

**Allowables for Tension:**
* **Tensile Rupture:** $$\sigma_n = 0.5 \times F_u$$
* **Tensile Yielding:** $$\sigma_n = 0.6 \times F_y$$

**Shear Stresses at Section II-II:**
* **In-plane Shear:** $$\tau_{xy} = \frac{F_1 \cos(\alpha)}{A_{II-II}}$$
* **Out-of-plane Shear:** $$\tau_{xz} = \frac{F_2}{A_{II-II}}$$
* **Governing Shear Stress:** $$\tau_u = \max(\tau_{xy}, \tau_{xz})$$

**Allowables for Shear:**
* **Shear Rupture:** $$\sigma_{ns} = 0.3 \times F_u$$
* **Shear Yielding:** $$\sigma_{ns} = 0.4 \times F_y$$
* **Combined Tension & Shear** (AISC Eqn. C-H3-8): 
...
* **Required Shear Stress:** $$\tau_u = \frac{F_1}{2 \times A_{III-III}}$$
* **Allowable Shear Rupture:** $$\sigma_{ns} = 0.3 \times F_u$$
* **Allowable Shear Yielding:** $$\sigma_{ns} = 0.4 \times F_y$$
...
* **Bearing Area:** $$A_{pb} = 2R_3(t_1 + 2t_2)$$
* **Required Bearing Stress:** $$\sigma_u = \frac{F_1}{A_{pb}}$$
* **Allowable Bearing Strength:** $$\sigma_n = 0.9 \times F_y$$
...
* **Required Weld Stress:** $$\tau_u = \frac{F_1}{A_w}$$
* **Allowable Weld Stress** (Directional Increase): 
    $$F_w = \frac{0.6F_y (1 + 0.5 \sin^{1.5}(\alpha))}{\Omega_{weld}}$$
    *(Note: The code uses an effective factor of 2 for $\Omega_{weld}$ to derive the working allowable).*
* **Unity Check:** $$\frac{\tau_u}{F_w} \le 1.0$$
